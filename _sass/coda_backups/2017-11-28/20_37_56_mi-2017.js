...
markdown.convert = function( node ){
	if(node.nodeType == 3){
		node.parentNode.innerHTML = markdown.converter.makeHtml(node.nodeValue);
		console.log(markdown.converter.makeHtml(node.nodeValue));
	}
}
markdown.init = function() {
	
	var nodes = Array.prototype.slice.call(document.querySelectorAll("[markdown]"));
	
	while(node = nodes.shift()){
		if(node.childNodes){
			Array.prototype.slice.call(node.childNodes).forEach(function(ele){
				nodes.push(ele);
			});
		}
		markdown.convert(node);
	}
}
